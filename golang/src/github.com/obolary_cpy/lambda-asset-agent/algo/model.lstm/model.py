...
class Model( tf.keras.Model ):
    ...
    def __init__( self, **kwargs ):
        ...
        self.common = km.Sequential([
            # LSTM expects (timesteps,features)
            kl.Reshape( (256,1) ),
            kl.LayerNormalization(),
            kl.Bidirectional( kl.LSTM( 256, return_sequences=True ) ),
            kl.Bidirectional( kl.LSTM( 512, return_sequences=True ) ),
        ])

        # policy logits
        self.hidden1 = km.Sequential([
            kl.Bidirectional( kl.LSTM( 1024, return_sequences=True ) ),
            kl.Bidirectional( kl.LSTM( 2048 ) ),
        ])
...
class A2CAgent:
    ...
    def __init__(self, model):
        ...
        self.params = { 'value': 0.5, 'entropy': 0.001, 'gamma': 0.99 }
        ...
        self.model.compile(
            ...
            optimizer = koz.RMSprop( lr=0.001 ),
...
def train( dataset_file, reward_func='mean' ):
    ...
    for i in range( 10000 ): 
        ...
        raw_ep = agent.train( env, updates=updates, batch_sz=32 )
...
'''  
model = Model(num_actions= 1)
agent = A2CAgent(model)
rewards_history = train( dataset_file= '/Users/joshua/source/golang/src/github.com/obolary/lambda-asset-corpus/ALL/XAUUSD_1M_ALL.csv')
...
train( '/Users/joshua/source/golang/src/github.com/obolary/lambda-asset-corpus/ALL/XAUUSD_1M_ALL.csv', reward_func )
