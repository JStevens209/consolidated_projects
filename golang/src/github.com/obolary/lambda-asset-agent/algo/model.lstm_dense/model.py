...
class A2CAgent:
    ...
    def train( self, env, batch_sz=32, updates=1024, override_action_value=False, update_frequency=100, update_graph_path=None ):

        # read weights if available
        if update_graph_path != None and os.path.exists( update_graph_path ):
            self.model.load_weights( update_graph_path )
        ...
        for update in range( updates ):
            ...
            for step in range( batch_sz ):
                ...
                if override_action_value:
                    actions[step] = env.expected_action()
            ...
            if update % update_frequency == 0:
                print( '** step {}: losses(acts,advs,rets) {}'.format( update, losses ) )
                if update_graph_path != None:
                    self.model.save_weights( update_graph_path )
